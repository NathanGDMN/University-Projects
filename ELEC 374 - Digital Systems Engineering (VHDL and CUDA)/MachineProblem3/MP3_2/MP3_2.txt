...
/*
	2. Code __global__(GPU) function for matrixMultiplication but one thread calculates whole output matrix
*/
	...
	for (int i = 0; i < sizeOfMatricies; i++) {  		//i =row
		for (int j = 0; j < sizeOfMatricies; j++) {		//j =col
			...
			for (int k = 0; k < sizeOfMatricies; k++) {
				Pvalue += d_a[i*sizeOfMatricies + k] * d_b[k*sizeOfMatricies + j];
			}
			d_c[i*sizeOfMatricies + j] = Pvalue;
		}
	}

}

//Regular C function (to run on CPU) for matrixMultiplication:
...
int main(int argc, char *argv[]) {
	...
	const int dimOfMatricies = 125; //Value determines size of matricies Ex. dimOfMatricies = 5 will result in 5x5 matricies
	...
	int blckWidth = 1;
	int gridWidth = 1;
	/* Below is not needed for this part as it asks for one block and one thread
	...
	*/
	...
	int start_s = clock();	//Record time taken by CPU

	matrixMultiplication(a, b, d, dimOfMatricies); //d is output matrix
	
	int stop_s = clock();
	float cpu_time = ((stop_s - start_s) / double(CLOCKS_PER_SEC)) * 1000; //Calculate time taken by CPU
