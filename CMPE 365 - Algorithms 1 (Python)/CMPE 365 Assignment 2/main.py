...
def buildTristrips( triangles ): 
    ...
    while triangleCount < len(triangles):  #While there's more triangles to be checked
        #find not-checked triangle
        currentTriangle = triangles[i] 
        if inStrip(currentTriangle):
            i += 1
            continue    #If currentTriangle is inStrip, pick new one
        triangleCount = buildTristrip(triangleCount, currentTriangle, 1) #Build Tristrip forwards -- triangle.nextTri = newTri, newTri.prevTri = triangle
        if currentTriangle.prevTri == None: 
            triangleCount = buildTristrip(triangleCount, currentTriangle, 0) #Builds Tristrip backwards -- triangle.prevTri = newTri, newTri.nextTri = triangle 
        
        tristripCount += 1  # Increment 'count' every time you *start* a new triStrip.

    print( 'Generated %d tristrips' % tristripCount )
    print(triangleCount)

def buildTristrip(triangleCount, triangle, direction):
    print("Built initiated")
    print(triangleCount)
    triangleCount += 1
    newTriangleSet = False

    #---Find adj triangle not already in a tristrip
    for tri in triangle.adjTris:       
        if inStrip(tri):    
            ...
            newTriangleSet = True
            break
    #---Make sure new triangle is found
    if newTriangleSet == False:
        print("return run")
        return triangleCount  #Return if all adjTris are in a tristrip
        

    #---Add new triangle to linked list
    if direction == 1:
        triangle.nextTri = newTriangle
        newTriangle.prevTri = triangle
    else:
        triangle.prevTri = newTriangle
        newTriangle.nextTri = triangle

    triangleCount = buildTristrip(triangleCount, newTriangle, direction)
    return triangleCount 


def inStrip(triangle): #Checks if triangle is in tristrip
