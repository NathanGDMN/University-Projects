...
def buildTristrips( triangles ): 
    ...
    while len(checked) < len(triangles):  #While there's more triangles to be checked
    ...
    buildTristrip(checked, currentTriangle) #Build Tristrip forwards -- triangle.nextTri = newTri, newTri.prevTri = triangle 
        
    tristripCount += 1  # Increment 'count' every time you *start* a new triStrip.

    print( 'Generated %d tristrips' % tristripCount )

def buildTristrip(checked, triangle):
    checked.append(triangle)
