 ...
 * - add check for duplicate node names, right now assumes all node names are unique
 *
...
mutex nodeNamesMutex;
map<string,shared_ptr<Node>> nodeNames;
...
string Service::nodeName() {
    thread::id this_id = this_thread::get_id();
    shared_ptr<Node> this_node = getNode(this_id, "nodeName"s);
...
// node 
Node::Node(string nodeName):name(nodeName),alive(false){
...
void Node::startServices(){
   //cout << "in Node startServices" << endl;
   if (!alive) start();
...
void Node::init(){
    alive = true;
    //cout << "in Node::init() for node " << name << endl;
    map<string,shared_ptr<Node>>::iterator it;

    lock_guard<mutex> guard(nodeNamesMutex);
    it = nodeNames.find(name);
    if (it != nodeNames.end()){
        cerr << "Nodes must have unique names!!" << endl;
        cerr << "duplicate node name: " << name << endl;
	exit(1);
    }
    //cout << "adding name " << name << " to NodeNames" << endl;
    nodeNames.insert(make_pair(name, shared_from_this()));
}
...
void Node::cleanup(){
    //cerr << "Node: " << name << " in cleanup()" << endl;
    alive = false;
    // stop and wait any services on this node.
    stopServices();
    waitForServices();
    // name all sockets that are not service sockets and kill them
   for (int i = 0; i < numSockets; i++){
       if (sockets[i].owner == nullptr){
           sockets[i].messages.thread_stop(i + firstFD);
       }
   }
   // remove name from nodeNames
   // CHeck name is there first
   lock_guard<mutex> guard(nodeNamesMutex);
   //cerr << "check if Node " << name << " not in nodeNames?" << endl;
   if (nodeNames.find(name) == nodeNames.end()){
       //cerr << "Node " << name << " not in nodeNames?" << endl;
   } else {
       //cerr << "Removing node " << name << " rom nodeNames" << endl;
       nodeNames.erase(name);
   }
}
...
//   returns 0 or error EAI_NONAME (name not found) EAI_MEMORY (cannot allocate result) EAI_FAIL (parameters not valid)
//   currently does not support service name or hints
//-

int getaddrinfo(const char *nodeName, const char *service, const struct addrinfo *hints, struct addrinfo **res){
    stringstream ss;
    thread::id this_id = this_thread::get_id();
    shared_ptr<Node> this_node = getNode(this_id, "socket"s);
    shared_ptr<Service> this_service = getService(this_id, "socket"s);
    ...
    if (enterExit){
        ss << name << ": enter getaddrinfo" << endl;
        cout << ss.str();
    }

    // check parameters
    if (service != nullptr){
        ss << name << ": service name not supported in simulator" << endl;
	return EAI_FAIL;
    }
    if (hints != nullptr){
        ss << name << ": hints not supported in simulator" << endl;
	return EAI_FAIL;
    }
    if (res == nullptr){
        ss << name << ": need place to store results " << endl;
	return EAI_FAIL;
    }

    // node start and stop supposed to ensure only one node with the same name exists.
    map<string,shared_ptr<Node>>::iterator it;
   lock_guard<mutex> guard(nodeNamesMutex);
    it = nodeNames.find(nodeName);
    if (it == nodeNames.end()){
	// name not found
	return EAI_NONAME;
    }
    ...
    *res = new struct addrinfo;
    ...
    (*res) -> ai_flags = 0;
    (*res) -> ai_family = AF_INET; // only IPV4 supported
    (*res) -> ai_socktype = 0;
    (*res) -> ai_protocol = 0;
    ...
    struct sockaddr_in * saddr = new struct sockaddr_in;
	...
	*res = nullptr;
    ...
    saddr->sin_port = 0;
    saddr->sin_addr = it->second->address;


    if (enterExit){
        ss = stringstream();
        ss << name << ": exit getaddrinfo result = " << retVal <<  endl;
        cout  << ss.str();
    }
    return retVal;
}
...
const char *gai_strerror(int errcode){
	...
	case EAI_FAIL:
	    return "internal failure in getaddrinfo";
	case EAI_MEMORY:
