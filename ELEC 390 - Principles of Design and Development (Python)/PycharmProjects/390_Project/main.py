...
from sklearn.metrics import accuracy_score, f1_score, roc_curve, RocCurveDisplay,roc_auc_score
#For learning curve:
from sklearn.datasets import make_classification
from sklearn.model_selection import learning_curve
#Functions from additional files:
...
if debug == 1:
    ...
    print(numberOfSectionsTW)
    print(numberOfSectionsTJ)
    print(numberOfSectionsAW)
    print(numberOfSectionsAJ)
    print(numberOfSectionsNW)
    print(numberOfSectionsNJ)
...
for train_size, cv_train_scores, cv_test_scores in zip(train_size_abs, train_scores, test_scores):
    print(f"When {train_size} samples were used to train the model")
...
def process():
    ...
    for i in range(numberOfSec):
        feat = pd.concat([feat, getFeatures(split_data[i])])
        numSec.append(x)
        x = x + 1
